/* ulogd_VUURMUUR.c
 *
 * ulogd output target for Vuurmuur's traffic log format.
 *
	...
	for (i = 0; i < upi->input.num_keys; i++) {
		...
		case ULOGD_RET_STRING:
			if (strcmp(key->name, "oob.prefix") == 0) {
				...
				snprintf(vr.from_int, sizeof(vr.from_int), "in: %s ",
						(char *) key->u.value.ptr);
			} else if (strcmp(key->name, "oob.out") == 0) {
			    if (strlen((char *) key->u.value.ptr) > 0)
				snprintf(vr.to_int, sizeof(vr.to_int), "out: %s ",
						(char *) key->u.value.ptr);
				...
				if (vrmr_loglevel)
					vrmr_debug("ULOGD_RET_STRING", "key %s, %d, %s",
							key->name, key->type, (char *) key->u.value.ptr);
			}
			break;
		case ULOGD_RET_BOOL:
		case ULOGD_RET_INT8:
		...
		case ULOGD_RET_INT32:
			if (strcmp(key->name, "tcp.syn") == 0)
				...
				if (vrmr_loglevel)
					vrmr_debug("ULOGD_RET_BOOL/INT", "key %s, %d",
							key->name, key->type);
			}
			break;
		...
		case ULOGD_RET_UINT64:
			{
			    if (strcmp(key->name, "oob.family") == 0) {
					...
					if (vrmr_loglevel)
						vrmr_debug("ULOGD_RET_UINT", "key %s, %d",
								key->name, key->type);
				}
			    break;
			}
		case ULOGD_RET_IPADDR:
            if (vrmr_loglevel)
				vrmr_debug("ULOGD_RET_IPADDR", "key %s, %d",
						key->name, key->type);
			break;
		default:
...
static int vuurmuur_init(struct ulogd_pluginstance *upi)
{
	...
	op->of = fopen(upi->config_kset->ces[VUURMUUR_CONF_FILENAME].u.string, "a");
	if (!op->of) {
	...
	vrmr_loglevel = (int)upi->config_kset->ces[VUURMUUR_CONF_LOGLEVEL].u.value;

	vrmr_init(&vctx, "ulogd");
	vrmr_load(vrmr_loglevel, &vctx);

	vrmr_info("Info", "This is Ulogd2-Vuurmuur plugin %s", version);
	vrmr_info("Info", "Copyright (C) 2002-2013 by Victor Julien");

	vrmr_create_log_hash(vrmr_loglevel, &vctx, &service_htbl, &zone_htbl);
